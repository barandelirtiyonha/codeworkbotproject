...
exports.run = async (client, message, args) => {
  if(!message.member.roles.has(db.fetch(`kickyetkilisi_${message.guild.id}`))) {
  ...
  let kicklog = member.guild.channels.find("name", kicklogkanalı);
  if (message.mentions.users.size < 1) return message.reply('Kimi kickleyeceğimi yazmalısın.');
  if (reason.length < 0) return message.reply('Kick sebebini yazmalısın.');
  ...
  message.guild.channels.get(db.fetch(`kicklog_${message.guild.id}`)).send(embed);
};
