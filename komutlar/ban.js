...
exports.run = async (client, message, args) => {
 if(!message.member.roles.has(db.fetch(`banyetkilisi_${message.guild.id}`))) {
  ...
  let banlog = member.guild.channels.find("name", banlogkanalı);
  ...
  message.guild.channels.get(db.fetch(`banlog_${message.guild.id}`)).send(embed);
};
